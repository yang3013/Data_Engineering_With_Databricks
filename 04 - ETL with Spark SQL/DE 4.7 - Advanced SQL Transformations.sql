...
-- MAGIC %python
-- MAGIC schema = StructType.fromJson(json_str)
-- MAGIC # json_in
...
-- MAGIC import json
-- MAGIC json_str = spark.sql("SELECT value FROM events_strings LIMIT 1").collect()[0][0]
-- MAGIC schema_json = json.loads(json_str)
-- MAGIC schema = f.schema_of_json(json_str)
-- MAGIC df = spark.sql("SELECT value FROM events_strings").withColumn("converted_value", f.from_json(f.col("value"), schema))
