...
-- Show all schemas(databases) available
SHOW SCHEMAS

-- COMMAND ----------

-- Show tables from non-default database
SHOW TABLES

-- COMMAND ----------

-- Show tables from non-default database aka schema
SHOW TABLES FROM 
odl_instructor_605132_e2m3_da_dewd

-- COMMAND ----------

SELECT *, INPUT_FILE_NAME() FROM students
ORDER BY id
...
SELECT *, INPUT_FILE_NAME() FROM students
ORDER BY id
-- The file where id 4-6 belongs 
-- (dbfs:/mnt/dbacademy-users/odl_user_830219@databrickslabs.com/data-engineering-with-databricks/database.db/students/part-00000-b2104c3c-9f0f-48a3-98be-bf320f148cf4-c000.snappy.parquet) rewrote into a new file
...
-- MAGIC %fs ls dbfs:/mnt/dbacademy-users/odl_user_830219@databrickslabs.com/data-engineering-with-databricks/database.db/students

-- COMMAND ----------

-- MAGIC %python
-- MAGIC dbutils.fs.ls('dbfs:/mnt/dbacademy-users/odl_user_830219@databrickslabs.com/data-engineering-with-databricks/database.db/students')

-- COMMAND ----------

-- MAGIC %python
-- MAGIC spark.conf.set("spark.databricks.delta.formatCheck.enabled", False)
-- MAGIC display(spark.read.format('parquet').load('dbfs:/mnt/dbacademy-users/odl_user_830219@databrickslabs.com/data-engineering-with-databricks/database.db/students/part-00000-f264cc56-e768-4e1c-8038-f116f61ac8d7-c000.snappy.parquet'))

-- COMMAND ----------

-- MAGIC %python
-- MAGIC spark.conf.set("spark.databricks.delta.formatCheck.enabled", True)
...
SELECT *, INPUT_FILE_NAME() FROM updates;
