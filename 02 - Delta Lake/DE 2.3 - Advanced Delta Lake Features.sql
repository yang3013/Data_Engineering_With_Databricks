...
-- MAGIC display(spark.read.format('parquet').load("dbfs:/mnt/dbacademy-users/odl_user_830219@databrickslabs.com/data-engineering-with-databricks/database.db/students/part-00000-1ba96469-7175-4384-9ebe-75afaca458d4-c000.snappy.parquet"))
...
SELECT * FROM json.`${DA.paths.user_db}/students/_delta_log/00000000000000000007.json`
...
-- The previous small files are not deleted until VACUUM
SELECT *, input_file_name() FROM students
...
-- MAGIC %python
-- MAGIC spark.catalog.getTable("students")
...
-- MAGIC  
-- MAGIC deltaTable = DeltaTable.forPath(spark, f"{DA.paths.user_db}/students/")
-- MAGIC fullHistoryDF = deltaTable.history()
