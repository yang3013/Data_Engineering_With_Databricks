...
# MAGIC -- ANSWER
# MAGIC 
# MAGIC SHOW FUNCTIONS LIKE ".?current.*";
...
# MAGIC 
# MAGIC -- Alternative:
# MAGIC -- SHOW TABLES FROM sampledb;
...
# MAGIC 
# MAGIC DESCRIBE EXTENDED table1;
...
## To run, describe extended and copy the 
## location into dbutils.fs.ls()

## Programmatically the following statement
## extracts the table location by filtering and collect

tableLocation = spark.sql("DESCRIBE EXTENDED table1").filter(col("col_name") == 'Location').select("data_type").collect()[0][0]
dbutils.fs.ls(tableLocation)
