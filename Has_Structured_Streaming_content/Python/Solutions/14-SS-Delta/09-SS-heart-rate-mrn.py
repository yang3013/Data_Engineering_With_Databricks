...
# MAGIC -- ANSWER
# MAGIC 
# MAGIC CREATE OR REPLACE TEMP VIEW TEMP_recordings_mrn AS (
# MAGIC   SELECT b.mrn, a.device_id, a.time, a.heartrate
# MAGIC   FROM TEMP_heart_rate_silver a
# MAGIC   INNER JOIN
# MAGIC   charts_valid b
# MAGIC   ON a.device_id=b.device_id
# MAGIC   WHERE heartrate > 0
# MAGIC     AND ((time > start_time AND time < end_time) 
# MAGIC     OR (time > start_time AND valid = true))
# MAGIC   )
...
(spark.table("TEMP_recordings_mrn")
  .writeStream
  .format("delta")
  .outputMode("append")
  .option("checkpointLocation", Paths.recordingsMRNCheckpoint)
  .trigger(once=True)
  .start(Paths.recordingsMRNTable)).awaitTermination()
