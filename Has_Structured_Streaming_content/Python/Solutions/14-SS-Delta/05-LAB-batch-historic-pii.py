...
(spark.read
  .schema(schema)
  .csv(source)
  .createOrReplaceTempView("pii_export"))
...
# MAGIC -- ANSWER
# MAGIC 
# MAGIC SELECT mrn, dob, sex, gender, first_name, last_name, street_address, zip, city, state, from_unixtime(updated) updated
# MAGIC FROM pii_export
...
spark.sql(f"""
CREATE TABLE pii_current
USING delta
LOCATION '{Paths.piiCurrentPath}'
AS (
  SELECT mrn, dob, sex, gender, first_name, last_name, street_address, zip, city, state, from_unixtime(updated) updated
  FROM pii_export
)""")
